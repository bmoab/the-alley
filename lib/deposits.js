...
export async function resolveDeposit(id, action, actor = SYSTEM) {
  ...
  let noPayment = false;
  if (action === "refund") {
    const { refundId, simulated, noPayment: np } = await refundDeposit(booking);
    noPayment = Boolean(np);
    console.log(
      noPayment
        ? `[deposits] Resolved deposit for booking #${id} — no Square payment to refund (refund manually if collected).`
    ...
    logActivity({
      ...
      description: noPayment
        ? "Deposit marked refunded · no Square payment found (refund manually)"
        : `Deposit refunded · ${formatMoney(booking.deposit)}`,
      amount: noPayment ? null : booking.deposit,
      metadata: noPayment ? { no_payment_found: true } : null,
      ...actor,
    });
  } else {
    db.prepare(
  ...
  return { booking: getBooking(id), noPayment };
}
