...
/**
 * If `date` (YYYY-MM-DD) is TODAY in America/Denver (the venue's timezone),
 * return the current fractional hour in Denver (e.g. 14.5 = 2:30 PM). Otherwise
 * null. Used to hide/disable already-passed start times for same-day bookings —
 * the server runs in UTC, so we must compare against Denver wall-clock time.
 */
function denverPastCutoff(date) {
  ...
  if (date !== `${p.year}-${p.month}-${p.day}`) return null;
  const hour = Number(p.hour) % 24; // some platforms emit "24" at midnight
  return hour + Number(p.minute) / 60;
}

/** True if `startTime` on `date` has already passed (Denver), for today only. */
export function isStartInPast(date, startTime) {
  const cutoff = denverPastCutoff(date);
  return cutoff != null && toHour(startTime) <= cutoff;
}
...
export function getAvailableStartTimes(space, date, hours = 2) {
  ...
  const pastCutoff = denverPastCutoff(date); // null unless `date` is today (Denver)
  ...
  for (let h = openHour; h + duration <= closeHour + 1e-9; h += 0.5) {
    ...
    // A slot is available if it overlaps no occupied interval and (for today)
    // hasn't already passed.
    const conflict = occupied.some(
      (iv) => slotStart < iv.end && slotEnd > iv.start
    );
    const past = pastCutoff != null && slotStart <= pastCutoff;
...
export function getDayFreeSlots(space, date) {
  ...
  const closeHour = Number(getSetting("close_hour", "23"));
  const occupied = getBlockedIntervals(space, date);
  const pastCutoff = denverPastCutoff(date);
  const slots = [];
  for (let h = openHour; h <= closeHour + 1e-9; h += 0.5) {
    ...
    // Free if this 30-min block overlaps no occupied interval and (for today)
    // hasn't already passed.
    const past = pastCutoff != null && h <= pastCutoff;
...
export function isSlotAvailable(space, date, startTime, hours) {
  if (isStartInPast(date, startTime)) return false; // can't book a passed time today
  const occupied = getBlockedIntervals(space, date);
