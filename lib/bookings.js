...
export function keepBookingOnCalendar(id) {
  db.prepare(
    "UPDATE bookings SET hold_expires_at = NULL WHERE id = ? AND status = 'held'"
  ).run(id);
  return getBooking(id);
}

/** Reverse keepBookingOnCalendar: re-arm the payment window from now. */
export function reArmHold(id) {
  const windowDays = Number(getSetting("payment_window_days", "3")) || 3;
  const expires = new Date();
  expires.setDate(expires.getDate() + windowDays);
  db.prepare(
    "UPDATE bookings SET hold_expires_at = ? WHERE id = ? AND status = 'held'"
