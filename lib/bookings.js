...
import { spaceBufferHours, spaceRules } from "./spaces.js";
 ...
 *
 * The buffer is the space's own (see lib/spaces.js) — correct by construction,
 * since only bookings in the same space are queried here.
 */
export function getOccupiedIntervals(space, date, excludeId = null) {
  const placeholders = BLOCKING_STATUSES.map(() => "?").join(",");
  // `(? IS NULL OR id != ?)` keeps the SQL string constant so better-sqlite3's
  // statement cache still applies and the parameter count never varies.
  const rows = db
    .prepare(
    ...
    .all(space, date, ...BLOCKING_STATUSES, excludeId ?? null, excludeId ?? null);

  const bufferHours = spaceBufferHours(space);
 ...
 */
export function getBlockedIntervals(space, date, excludeId = null) {
  return [...getOccupiedIntervals(space, date, excludeId), ...getClosureIntervals(space, date)];
}
 ...
 */
export function getAvailableStartTimes(space, date, hours = 2, excludeId = null) {
  const openHour = Number(getSetting("open_hour", "8"));
  const closeHour = Number(getSetting("close_hour", "23"));
  const occupied = getBlockedIntervals(space, date, excludeId);
 ...
 */
export function getDayAvailability(space, date, hours = 2, excludeId = null) {
 ...
 */
export function getDayBookings(space, date, excludeId = null) {
  const placeholders = BLOCKING_STATUSES.map(() => "?").join(",");
  const rows = db
    .prepare(
      `SELECT start_time, hours FROM bookings
 ...
 */
export function getDayFreeSlots(space, date, excludeId = null) {
  const openHour = Number(getSetting("open_hour", "8"));
  const closeHour = Number(getSetting("close_hour", "23"));
  const occupied = getBlockedIntervals(space, date, excludeId);
  const earliest = earliestBookableHour(date);
 ...
 */
export function isSlotAvailable(space, date, startTime, hours, excludeId = null) {
...
// ---------------------------------------------------------------------------
// Rescheduling: tokens, eligibility, conflict check, and the row mutation.
// The async orchestration (emails, activity log, events-row sync) lives in
// lib/reschedule.js so this module stays free of a catalog/email import cycle.
// ---------------------------------------------------------------------------

/** Statuses whose date may be moved. Mirrors the admin menu's "live" set. */
export const RESCHEDULABLE_STATUSES = ["held", "reserved", "confirmed"];

/**
 * The private token behind a client's "change my date" link, minted on demand.
 * Mirrors ensureDirectoryToken / ensureExhibitorToken.
 */
export function ensureRescheduleToken(id) {
  const b = getBooking(id);
  if (!b) return null;
  if (b.reschedule_token) return b.reschedule_token;
  const token = nanoid(24);
  db.prepare("UPDATE bookings SET reschedule_token = ? WHERE id = ?").run(token, id);
  return token;
}

export function getBookingByRescheduleToken(token) {
  if (!token) return null;
  return db.prepare("SELECT * FROM bookings WHERE reschedule_token = ?").get(token);
}

/** How many days of notice a client needs to move their own booking. */
export function rescheduleNoticeDays() {
  const n = Number(getSetting("reschedule_notice_days", "14"));
  return Number.isFinite(n) && n >= 0 ? n : 14;
}

/** Whole days from today (venue-local) until `date`. Negative = in the past. */
...
/**
 * Whether this booking may be moved at all, and what the caller should be told.
 * `source: "client"` enforces the notice window as a hard gate; "admin" only
 * reports it as a warning the caller may override.
 *
 * Returns { ok, code?, error?, warnings[], noticeDays, daysOut }.
 */
export function canReschedule(booking, { source = "admin", now = new Date() } = {}) {
  const warnings = [];
  const noticeDays = rescheduleNoticeDays();
  if (!booking) return { ok: false, code: "not_found", error: "Booking not found.", warnings, noticeDays };
  if (booking.archived) {
    return { ok: false, code: "archived", error: "This booking is archived.", warnings, noticeDays };
  }
    ...
    return {
      ...
      error:
        booking.status === "pending"
          ? "This request hasn't been approved yet, so there's no reserved date to move."
          : `A ${booking.status} booking can't be moved.`,
      warnings,
      noticeDays,
    };
  }

  const daysOut = daysUntil(booking.date, now);
  if (daysOut < 0) {
    return { ok: false, code: "past", error: "This booking has already happened.", warnings, noticeDays, daysOut };
  }
  if (daysOut < noticeDays) {
    if (source === "client") {
      return {
        ok: false,
        code: "too_close",
        error: `Changes need at least ${noticeDays} days' notice.`,
        warnings,
        noticeDays,
        daysOut,
      };
  ...
  if (source === "client") {
    const cap = Number(getSetting("reschedule_max_client_moves", "2")) || 2;
    if ((Number(booking.reschedule_count) || 0) >= cap) {
      return {
        ok: false,
        code: "too_many_moves",
        error: "This booking has already been moved a couple of times — let's sort the next change together.",
        warnings,
        noticeDays,
        daysOut,
      };
  ...
  return { ok: true, warnings, noticeDays, daysOut };
}

/**
 * Can `booking` move to date/startTime? Self-excluding, so "same day, two hours
 * earlier" works — the booking's own slot is invisible to the check.
 *
 * Deliberately NOT reusing findRestoreConflict: its status list is hardcoded to
 * ['held','confirmed'] (missing `reserved`, i.e. every recurring-series date)
 * and it ignores closures entirely.
 *
 * Returns null when clear, else { code, conflict? }.
 */
export function findRescheduleConflict(booking, date, startTime) {
  const hours = Number(booking.hours) || 0;
  if (isStartTooSoon(date, startTime)) return { code: "too_soon" };
  ...
  const openHour = Number(getSetting("open_hour", "8"));
  const closeHour = Number(getSetting("close_hour", "23"));
  if (start < openHour || end > closeHour + 1e-9) return { code: "outside_hours" };

  if (getClosureIntervals(booking.space, date).some((iv) => start < iv.end && end > iv.start)) {
    return { code: "closed" };
  }

  const occupied = getOccupiedIntervals(booking.space, date, booking.id);
  if (!occupied.some((iv) => start < iv.end && end > iv.start)) return null;

  // Name the culprit so the admin error can say who it clashes with.
  const buffer = spaceBufferHours(booking.space);
  const placeholders = BLOCKING_STATUSES.map(() => "?").join(",");
  const others = db
    .prepare(
      `SELECT * FROM bookings
        WHERE space = ? AND date = ? AND id != ? AND status IN (${placeholders})`
    )
    .all(booking.space, date, booking.id, ...BLOCKING_STATUSES);
  const conflict = others.find((o) => {
    const oStart = toHour(o.start_time);
    const oEnd = oStart + Number(o.hours) + buffer;
    return start < oEnd && end > oStart - buffer;
  });
  return { code: "taken", conflict: conflict || null };
}

/** Recompute a series' end date across every row (it gates deposit release). */
export function recomputeSeriesEndDate(seriesId) {
  db.prepare(
    `UPDATE bookings SET series_end_date = (SELECT MAX(date) FROM bookings WHERE series_id = ?)
      WHERE series_id = ?`
  ).run(seriesId, seriesId);
}

/**
 * Move a booking's date/start time. Synchronous and row-only — the caller
 * (lib/reschedule.js) handles validation, the events row, emails and logging.
 *
 * Re-checks the conflict INSIDE the transaction: that's the point of the
 * immediate lock, closing the gap between two admins (or an admin and a client
 * link) aiming at the same slot.
 */
export function applyBookingMove(id, { date, start_time }) {
  const move = db.transaction((bookingId, toDate, toStart) => {
    const b = db.prepare("SELECT * FROM bookings WHERE id = ?").get(bookingId);
    if (!b) return { ok: false, code: "not_found" };

    const conflict = findRescheduleConflict(b, toDate, toStart);
    ...
    db.prepare(
      `UPDATE bookings
        ...
        WHERE id = ?`
    ).run(toDate, toStart, bookingId);

    // A held booking only survived this long because the expiry sweep hadn't
    // run yet. releaseExpiredHolds() fires on EVERY /admin/bookings load and
    // sweeps on (payment window lapsed OR event date passed) — so without this
    // re-arm the move would succeed and then silently undo itself.
    let rearmed = false;
    if (b.status === "held" && !b.keep_on_calendar && b.hold_expires_at) {
      if (new Date(b.hold_expires_at).getTime() < Date.now()) {
        const days = Number(getSetting("payment_window_days", "3")) || 3;
        const until = new Date(Date.now() + days * 86400000).toISOString();
        db.prepare(
          "UPDATE bookings SET hold_expires_at = ?, payment_reminder_last_sent = ? WHERE id = ?"
        ).run(until, venueToday(), bookingId);
        rearmed = true;
      }
    }

    if (b.series_id) recomputeSeriesEndDate(b.series_id);

    return {
      ok: true,
      rearmed,
      before: { date: b.date, start_time: b.start_time },
      booking: db.prepare("SELECT * FROM bookings WHERE id = ?").get(bookingId),
    };
  });
  return move.immediate(id, date, start_time);
}

/** Insert a new pending booking request. Returns the new row id. */
export function createBookingRequest(data) {
  const { rate, deposit } = spaceRules(data.space);
 ...
 * { date, start_time, hours, session_title? } occurrences. ONE deposit lives on the first row
...
export function createBookingSeries(data, dates) {
  ...
  const insert = db.prepare(`
    INSERT INTO bookings (
      space, date, start_time, hours, status,
      client_name, client_email, client_phone,
      event_title, session_title, event_type, guests, alcohol, notes,
      is_recurring, recurring_schedule, is_public_event,
      rate, sessions, deposit, total,
  ...
  const tx = db.transaction(() => {
    ...
    dates.forEach((occ, i) => {
      ...
      const info = insert.run({
        ...
        client_phone: data.client_phone,
        event_title: data.event_title ?? null,
        // Per-occurrence, unlike everything else here — the client may have
        // given each session its own name.
        session_title: occ.session_title ?? null,
        event_type: data.event_type ?? null,
        guests: data.guests ?? null,
        alcohol: data.alcohol ? 1 : 0,
        notes: data.notes ?? null,
...
export function listBookings({
  ...
  // A single status, or an array of them (the admin's "Active" filter passes
  // several at once). An empty array would match nothing, so it's ignored.
...
/**
 * Every booking made under one email address — the client portal's whole data
 * set, and the only query it runs.
 *
 * Archived rows are excluded: archiving is the owner's "hide this from my list",
 * and the client has even less reason to see it. Emails are folded to lowercase
 * because nothing normalizes client_email on the way in (see lib/portal.js).
 */
export function listBookingsForClient(email) {
...
// A booking the client can still act on vs. one that's done vs. one that isn't
// happening. 'pending' is live here (it's a request they're waiting to hear
// about) even though it holds no slot, which is why this isn't BLOCKING_STATUSES.
const PORTAL_LIVE_STATUSES = ["pending", "held", "reserved", "confirmed"];
const PORTAL_DEAD_STATUSES = ["cancelled", "denied", "expired"];

/**
 * What this booking still owes.
 *
 * ⚠️ `total` alone can't answer this for a recurring series' holder. A series
 * is billed as TWO separate invoices — the session rentals roll out one per
 * date (payment_status), while the single cleaning deposit is invoiced on its
 * own at approval (deposit_payment_status) — yet the holder's `total` is
 * rental + deposit. Reading `total` there would call the whole thing unpaid
 * while the deposit sits paid, or vice versa. Single bookings really are one
 * invoice for the lot, so `total` is right for them.
 */
export function amountOutstanding(booking) {
  if (!booking) return 0;
  if (!PORTAL_LIVE_STATUSES.includes(booking.status)) return 0;
  // A pending request is a request: not approved, not priced, not invoiced. Its
  // `total` is only the estimate the booking form produced, so counting it as
  // money owed would bill someone for a date we haven't agreed to yet.
  if (booking.status === "pending") return 0;
  const settled = (s) => s === "paid" || s === "refunded";

  if (booking.series_id && booking.is_deposit_holder) {
    const rental = settled(booking.payment_status) ? 0 : rentalAmount(booking);
    const dep = settled(booking.deposit_payment_status) ? 0 : Number(booking.deposit) || 0;
    return Math.round((rental + dep) * 100) / 100;
  }
  return settled(booking.payment_status) ? 0 : Number(booking.total) || 0;
}

/**
 * Shape one client's bookings for their portal.
 *
 * A recurring series is collapsed into ONE entry with its sessions inside —
 * otherwise a 12-week course reads as twelve unrelated bookings, which is
 * precisely the confusion this page exists to fix (each session is invoiced
 * separately on a rolling basis, so they really do each have their own
 * paid/unpaid state).
 *
 * Returns { upcoming, past, inactive, outstanding } where the first three are
 * arrays of entries: { kind: "single" | "series", ... }.
 */
export function groupClientBookings(rows, { today = venueToday() } = {}) {
  const singles = [];
  const seriesById = new Map();

  for (const b of rows) {
    if (!b.series_id) {
      singles.push({ kind: "single", key: `b${b.id}`, booking: b, sessions: [b] });
      continue;
    }
    if (!seriesById.has(b.series_id)) {
      seriesById.set(b.series_id, { kind: "series", key: `s${b.series_id}`, sessions: [] });
    }
    seriesById.get(b.series_id).sessions.push(b);
  }

  for (const s of seriesById.values()) {
    s.sessions.sort((a, b) => a.date.localeCompare(b.date));
    // The holder carries the series' deposit and its title, and is what the
    // public listing hangs off — so it's the entry's representative row.
    s.booking = s.sessions.find((r) => r.is_deposit_holder) || s.sessions[0];
  }

  const entries = [...singles, ...seriesById.values()];
  const upcoming = [];
  const past = [];
  const inactive = [];

  for (const e of entries) {
    // "Standing" is the inverse of cancelled/denied/expired, NOT the live set —
    // a completed booking is one that HAPPENED, and belongs under "already
    // happened" rather than "not going ahead".
    const standing = e.sessions.filter((r) => !PORTAL_DEAD_STATUSES.includes(r.status));
    // A series is "upcoming" while ANY session is still ahead — a course whose
    // first weeks have passed hasn't finished.
    const ahead = standing.filter((r) => r.date >= today);
    e.outstanding = e.sessions.reduce((sum, r) => sum + amountOutstanding(r), 0);
    e.nextDate = ahead.length ? ahead[0].date : null;
    e.lastDate = e.sessions.length ? e.sessions[e.sessions.length - 1].date : null;

    if (!standing.length) inactive.push(e);
    else if (ahead.length) upcoming.push(e);
    else past.push(e);
  }

  upcoming.sort((a, b) => (a.nextDate || "").localeCompare(b.nextDate || ""));
  past.sort((a, b) => (b.lastDate || "").localeCompare(a.lastDate || ""));
  inactive.sort((a, b) => (b.lastDate || "").localeCompare(a.lastDate || ""));

  return {
    upcoming,
    past,
    inactive,
    outstanding: upcoming.reduce((sum, e) => sum + e.outstanding, 0),
  };
}
...
export function approveBooking(id, pricing) {
  ...
  // Mint the change-date token before the approval email is built, so the link
  // is present on the very first email the client gets.
...
/**
 * Flip whether a booking wants a spot on the public calendar — the box the
 * client ticks (or doesn't) when they book, which nothing could change
 * afterwards until now.
 *
 * Worth setting even on an unpaid booking: confirmBookingPaid reads this flag
 * to decide whether to create the listing and email the host their link, so
 * turning it on early means payment does the work with the right wording.
 *
 * A recurring series is ONE decision — the listing lives on the holder session
 * and fans out across the rest — so every session carries the same flag and no
 * badge contradicts its siblings.
 */
export function setBookingPublicFlag(id, on) {
  const booking = getBooking(id);
  if (!booking) return null;
  const flag = on ? 1 : 0;
  if (booking.series_id) {
    db.prepare("UPDATE bookings SET is_public_event = ? WHERE series_id = ?").run(flag, booking.series_id);
  } else {
    db.prepare("UPDATE bookings SET is_public_event = ? WHERE id = ?").run(flag, id);
  }
  return getBooking(id);
}

/**
 * Owner override: keep an approved-but-unpaid hold on the calendar indefinitely
 * by clearing its expiry. releaseExpiredHolds only sweeps holds with a non-null
 * past hold_expires_at, so a null expiry never lapses — the slot stays reserved
 * while a deal is worked out. Reversible via reArmHold.
 */
export function keepBookingOnCalendar(id) {
  // Works before OR after auto-expiry: on a still-held booking it just stops the
  // countdown; on one that already expired it restores it to held. Either way it
  // sets the flag + clears the deadline so no sweep touches it again.
  db.prepare(
    "UPDATE bookings SET keep_on_calendar = 1, hold_expires_at = NULL, status = 'held' WHERE id = ? AND status IN ('held', 'expired')"
  ).run(id);
  return getBooking(id);
}

/** Reverse keepBookingOnCalendar: clear the flag + re-arm the payment window. */
export function reArmHold(id) {
  const windowDays = Number(getSetting("payment_window_days", "3")) || 3;
  const expires = new Date();
  expires.setDate(expires.getDate() + windowDays);
  db.prepare(
    "UPDATE bookings SET keep_on_calendar = 0, hold_expires_at = ? WHERE id = ? AND status = 'held'"
...
/**
 * Held, unpaid bookings that should auto-expire — UNLESS kept on the calendar.
 * Two triggers: the payment window lapsed (hold_expires_at in the past), or the
 * event date itself has passed (a deadline-less hold that was never resolved).
 * keep_on_calendar = 1 exempts a booking from both.
 */
export function listExpiredHolds(now = new Date()) {
  ...
  return db
    .prepare(
      `SELECT * FROM bookings
        WHERE status = 'held' AND payment_status != 'paid' AND keep_on_calendar = 0
          AND (
...
/**
 * Check whether restoring `booking` would collide with anything now standing in
 * its old slot. Returns null when clear, else:
 *   { kind: "booking", booking }  — another reservation is in the way
 *   { kind: "closure", closure }  — the owner has since closed that time
 *
 * Two bugs lived here: the status list was hardcoded to ('held','confirmed'),
 * which silently ignored `reserved` — i.e. EVERY recurring-series session — and
 * `completed`; and closures weren't consulted at all, so a booking could be
 * restored onto a day the owner had since closed. Both now go through the same
 * vocabulary as every other availability check (BLOCKING_STATUSES + closures).
 *
 * The restored booking's own slot is still expanded by the cleanup buffer on
 * BOTH sides — deliberately stricter than isSlotAvailable, which only buffers
 * after each existing booking. Restoring squeezes a booking back into a calendar
 * that has moved on, so erring toward "ask a human" is the right bias here.
 */
export function findRestoreConflict(booking) {
  if (!booking) return null;
  const bufferHours = spaceBufferHours(booking.space);

  const rawStart = toHour(booking.start_time);
  const rawEnd = rawStart + Number(booking.hours);
  const start = rawStart - bufferHours;
  const end = rawEnd + bufferHours;

  // Closures are hard blocks and carry no buffer of their own, so they're tested
  // against the booking's actual hours rather than the buffered span.
  const closure = getClosureIntervals(booking.space, booking.date).find(
    (iv) => rawStart < iv.end && rawEnd > iv.start
  );
  if (closure) return { kind: "closure", closure };
  ...
  const others = db
    ...
    .all(booking.space, booking.date, booking.id, ...BLOCKING_STATUSES);

  for (const o of others) {
    const oStart = toHour(o.start_time);
    const oEnd = oStart + Number(o.hours) + bufferHours; // buffer after the other
    if (start < oEnd && end > oStart) return { kind: "booking", booking: o };
  }
  return null;
}
