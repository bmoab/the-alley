...
export function ensureSeeded() {
  ...
  for (const o of OWNERS) {
    ...
    if (!existing) {
      ...
      // Bootstrap safety net: if this owner still hasn't completed first-login
      // setup (must_change_password = 1), (re)issue a fresh one-time temp
      // password and print it to the server log on each deploy. This guarantees
      // a working way in even when email delivery isn't configured yet. It stops
      // automatically once they set their own password. (Safe to remove this
      // block after both owners have onboarded.)
      if (existing.must_change_password) {
        ...
        console.log(
          `[auth] Owner ${o.name} <${o.email}> still needs first-login setup — temporary password: ${temp}`
        );
      }
    }
  }
...
// ---------------------------------------------------------------------------
// Roles & capabilities. Three tiers:
//   owner → everything, including managing the team (add/remove/change roles)
//   admin → all booking + content operations, but NOT the team
//   user  → view + edit site content, but NOT the booking money lifecycle
//           (approve/deny, mark paid, reissue/resend, cancel, deposits) or team
// ---------------------------------------------------------------------------
export const ROLES = ["owner", "admin", "user"];

/** Normalize an arbitrary role string to one of the known roles. */
...
/**
 * Owners and admins can act on bookings — approve/deny, take payment, reissue,
 * cancel, resolve deposits. A plain "user" cannot (this is the gate that stops
 * a limited account from approving or charging).
 */
...
/**
 * Resolve the current user IF they may manage bookings, else null. Server
 * actions call this and redirect with a permission notice when it's null — the
 * real enforcement, independent of whether the UI hid the control.
 */
export async function requireBookingManager() {
  const user = await getCurrentUser();
  return canManageBookings(user) ? user : null;
}
...
/** Change a user's role. Guards demoting the last active owner (to admin or user). */
export function setUserRole(id, role) {
  const safeRole = normalizeRole(role);
  if (safeRole !== "owner" && isLastActiveOwner(id)) {
