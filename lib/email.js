...
function seriesDatesHtml(rows) {
  return `<table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:14px">
    ${rows
      .map(
        (r, i) =>
          `<tr><td style="padding:6px 0;color:#5b5147">Session ${i + 1}</td><td style="padding:6px 0;text-align:right;font-weight:600;color:#1c1815">${formatDate(r.date)} · ${formatTime(r.start_time)}</td></tr>`
      )
      .join("")}
  </table>`;
}
...
export function emailOwnerNewSeriesRequest(holder, rows) {
  const html = shell(`
    <p>You have a new <strong>recurring</strong> booking request — ${rows.length} sessions.</p>
    <p><strong>${holder.client_name}</strong> would like to reserve ${spaceName(holder.space)}, ${holder.hours} hours each:</p>
