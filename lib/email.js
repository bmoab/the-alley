...
export async function sendEmail({ to, subject, html, text, attachments = [] }) {
  ...
  try {
    ...
    if (mode === "smtp") {
      ...
      await transport.sendMail({ from: FROM, to, subject, html, text: body, attachments });
      return { mode, ok: true };
    }
  } catch (err) {
    console.error(`[email] ${mode} send failed, logging instead:`, err.message);
    console.log(`\n✉️  (failed ${mode}) To: ${to} · ${subject}\n${body}\n`);
    return { mode: "console", ok: false };
  }
}
...
const RENTAL_AGREEMENT_PATH = path.join(process.cwd(), "public", "uploads", "rental-agreement.pdf");
...
export function emailClientDenied(b) {
  const html = shell(`
    <p>Hi ${firstName(b.client_name)},</p>
    <p>Thank you for thinking of The Alley for your ${b.event_type ? b.event_type.toLowerCase() : "event"}.
       Unfortunately, we&rsquo;re unable to accommodate this particular request${b.date ? ` for ${formatDate(b.date)}` : ""}.</p>
