...
// Where customer replies land. Defaults to OWNER (the monitored Gmail) so a
// no-reply@ From address still routes replies to a real inbox someone checks.
const REPLY_TO = process.env.EMAIL_REPLY_TO || OWNER;
...
export async function sendEmail({ to, subject, html, text, attachments = [] }) {
  ...
  try {
    ...
    if (mode === "smtp") {
      ...
      const info = await transport.sendMail({ from: FROM, replyTo: REPLY_TO, to, subject, html, text: body, attachments });
...
export function emailOwnerNewSeriesRequest(holder, rows) {
  const html = shell(`
    ...
    <p><a href="${APP_URL}/admin/requests" style="display:inline-block;background:#1c1815;color:#f7f1e6;text-decoration:none;padding:10px 20px;border-radius:999px;font-weight:600;font-size:14px">Review in admin →</a></p>
  `);
  return sendEmail({ to: OWNER, subject: `New recurring request (${rows.length} sessions) — ${holder.client_name}`, html });
}
