...
import { spaceName, formatDate, formatMoney, venueToday } from "./constants.js";
...
export async function confirmBookingPaid(id, actor = SYSTEM, { comped, notify = true } = {}) {
  ...
  // A comped booking skips approval entirely, so this may be the first chance
  // to mint the change-date token before the confirmation email is built.
  ...
  try {
    ...
    if (booking.is_public_event && (!booking.series_id || booking.is_deposit_holder)) {
      // The listing can already exist: the owner may have switched this booking
      // to public before it was paid. createHostListingDraft is idempotent and
      // returns that EXISTING row, so the invite must carry the row's own token
      // — emailing the token minted here would send a link never stored on any
      // row, and the host would land on "Link not found".
      const listing = createHostListingDraft(booking, nanoid(24));
      // If they've already filled the listing in, there's nothing to invite
      // them to do; re-sending would read as a request to start over.
      if (!listing.host_posted) {
        const inviteRes = await emailHostInvite(booking, listing.host_token);
        logEmail({
          bookingId: booking.id,
...
/**
 * Send a payment reminder to every client whose approved booking is still
 * unpaid and within its hold window, at most once per day per booking. Runs
 * daily from a cron; the approval day is skipped (approveBooking stamps it) so
 * the first nudge lands the day after approval. Returns the count sent.
 */
export async function runPaymentReminders(now = new Date()) {
  const due = listPaymentRemindersDue(now);
  let sent = 0;
  for (const b of due) {
    try {
      const res = await emailClientPaymentLink(b, { reason: "reminder" });
      markPaymentReminderSent(b.id, now);
      logEmail({
        bookingId: b.id,
        eventType: "payment_reminder_sent",
        description: `Payment reminder sent · ${formatMoney(b.total)}`,
      ...
      console.error(`[payments] payment reminder failed for #${b.id}:`, err.message);
    }
  }
  return sent;
}
...
export async function releaseExpiredHolds() {
  ...
  for (const b of expired) {
    expireBooking(b.id);
    const eventPassed = b.date && b.date < today;
    logActivity({
      bookingId: b.id,
      eventType: "hold_expired",
      description: eventPassed
        ? `Hold expired — event date (${formatDate(b.date)}) passed unpaid, ${spaceName(b.space)} reopened`
        : `Hold expired — ${spaceName(b.space)} on ${formatDate(b.date)} reopened`,
      ...SYSTEM,
    });
    // Only notify the client when the payment window lapsed BEFORE the event.
    // Emailing "your hold expired" about an event that already happened is
    // confusing, so skip it for the past-event cleanup case.
