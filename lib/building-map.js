 ...
 *   "conference" → the rentable conference room
 *   (none)       → a normal leasable suite
 * A zone with a `space` is bookable, not leasable — it renders as a booking
 * card and is excluded from tenant assignment in the admin directory.
 */
export const BUILDING_MAP = {
  front: {
    ...
    vb: "7 0 1756 1427",
    bands: [
      { floor: "upper", name: "Upper Floor", sub: "Studios & offices · 201–203, 205 + The Loft & Conference Room", x: 7, y: 0, w: 1756, h: 745 },
  ...
  floors: [
    ...
    },
    {
      ...
      // the floor was redrawn; that area is now the bookable Conference Room
      // (zone code "conference") with the Common Space label above it — the
      // Common Space is not interactive. Rects derived from Second floor.svg
      // wall geometry: the conference room's walls sit at x 292.38 → 693.39 and
      // y 532.99 → 814.28, inset ~6 to match the other rects (cf. 205).
      suites: [
        ...
        { code: "conference", x: 298, y: 539, w: 389, h: 269, kind: "conference", space: "conference" },
...
/** If a zone IS a rentable space, return its space id ("main"|"loft"|"conference"). */
