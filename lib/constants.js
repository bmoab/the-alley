...
export const SPACES = [
  ...
  {
    id: "conference",
    name: "The Alley Conference Room",
    // Capacity is intentionally blank — it's owner-editable in
    // admin → Settings → Per-space rules (see lib/spaces.js). Anything set
    // there wins; this value is only the fallback.
    location: "Upstairs",
    capacity: "",
    blurb:
      "A quieter room upstairs for the work that needs focus — board meetings, interviews, client sessions, and small trainings. Book it by the hour, for as little as an hour at a time.",
    features: ["Private, closed-door room", "Table & seating included", "Steps from the restrooms", "By the hour, 1-hour minimum"],
    gallery: [],
  },
];
...
/**
 * Now as "YYYY-MM-DD HH:MM" in the venue's timezone.
 *
 * Booking dates and start times are stored as bare venue-local wall clock, so
 * comparing them against a venue-local "now" string is apples to apples. Use
 * this (not `new Date().toISOString()`) whenever SQL needs to ask "has this
 * booking happened yet?" — the server runs in UTC, which is six or seven hours
 * ahead of Denver and rolls the date over mid-evening.
 */
export function venueNow(now = new Date()) {
  ...
  // Some platforms emit "24" for midnight rather than "00".
 ...
 * Prefer this over `venueToday(now + N * 86400000)` for "N days from today":
 * adding a fixed number of milliseconds drifts across DST boundaries, while
 * this walks the calendar.
 */
export function shiftDate(ymd, { days = 0, months = 0 }) {
  const [y, m, d] = ymd.split("-").map(Number);
...
export const DATE_PRESETS = [
  ...
  { key: "next30", label: "Next 30 days", range: (t) => ({ from: t, to: shiftDate(t, { days: 30 }) }) },
  { key: "next6m", label: "Next 6 months", range: (t) => ({ from: t, to: shiftDate(t, { months: 6 }) }) },
  { key: "next1y", label: "Next year", range: (t) => ({ from: t, to: shiftDate(t, { months: 12 }) }) },
  { key: "last30", label: "Last 30 days", range: (t) => ({ from: shiftDate(t, { days: -30 }), to: t }) },
  { key: "last6m", label: "Last 6 months", range: (t) => ({ from: shiftDate(t, { months: -6 }), to: t }) },
