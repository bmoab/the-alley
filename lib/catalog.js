...
export function createDirectoryEntry(d) {
  return db
    .prepare(
      `INSERT INTO directory (business_name, category, description, photo_path, contact_link, sort_order)
       VALUES (?, ?, ?, ?, ?, ?)`
    )
    .run(
      d.business_name,
...
export function updateDirectoryEntry(id, d) {
  db.prepare(
    `UPDATE directory
       SET business_name = ?, category = ?, description = ?, photo_path = ?, contact_link = ?, sort_order = ?
     WHERE id = ?`
  ).run(
    d.business_name,
    d.category ?? null,
