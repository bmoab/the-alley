...
import { confirmBookingPaid } from "./payments.js";
...
export async function runSeriesInvoices(now = new Date()) {
  ...
  for (const b of due) {
    ...
    try {
      const { invoiceId, paymentLink, comped } = await createBookingInvoice(b);
      // A $0 session has no invoice to send. Confirm it instead — otherwise it
      // stays 'reserved' with a null invoice id and this cron retries it every
      // day forever.
      if (comped) {
        await confirmBookingPaid(b.id, SYSTEM, { comped: true });
        console.log(`[series] session #${b.id} is $0 — confirmed free of charge, no invoice`);
        continue;
      }
      setInvoiceInfo(b.id, { invoiceId, paymentLink });
      const updated = getBooking(b.id);
      const res = await emailClientSeriesInvoice(updated);
      // One entry only: the email record carries the recipient + "sent" badge
      // and already shows in the timeline, so a second plain activity row with
      // the same text would just look like a duplicate.
      logEmail({
        bookingId: b.id,
