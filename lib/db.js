...
function seedDefaults(connection) {
  const settingDefaults = {
    ...
    series_max_occurrences: "8", // cap on classes in one recurring request
    series_max_span_days: "31", // recurrence can't stretch beyond ~1 month
  };
  ...
  // Guarded on tenant_id so an assigned suite is never silently orphaned.
  connection.prepare("DELETE FROM suites WHERE zone = '204' AND tenant_id IS NULL").run();
}
