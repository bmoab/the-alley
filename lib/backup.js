        ...
        html:
          ...
          (tooLargeToEmail
            ? `<p><strong>This archive (${Math.round(sizeKb / 1024)} MB) is over the ${MAX_EMAIL_MB} MB email cap, so it is NOT attached.</strong> ` +
              `It is saved on the server volume (last ${KEEP} kept under <code>/data/backups</code>) — download it from there, ` +
              `or set up off-site storage if archives have grown this large.</p>`
