...
export async function refundPayment(booking, amountDollars, reason = "Booking cancellation refund") {
  const amount = Number(amountDollars) || 0;
  if (amount <= 0) return { refundId: null, simulated: true, amount: 0 };
  ...
  if (!isSquareConfigured() || String(booking.square_invoice_id || "").startsWith("SIM-")) {
    ...
    console.log(
      `[square:SIMULATED] Would refund $${amount.toFixed(2)} for booking #${booking.id} ` +
        `(${reason}) → refundId=${refundId}`
    );
    return { refundId, simulated: true, amount };
  }

  const paymentId = await getInvoicePaymentId(booking.square_invoice_id);
  if (!paymentId) {
    // The booking is marked paid but Square has no captured payment on the
    // invoice (e.g. it was "Mark as paid"-ed manually, or paid by cash/check).
    // Don't throw — there's simply nothing to refund through Square. The caller
    // decides how to proceed (cancellation should still go through).
    console.warn(
      `[square] No captured payment on invoice ${booking.square_invoice_id} for booking #${booking.id} — nothing to refund.`
    );
  ...
  const refund = await squareFetch("/v2/refunds", {
    ...
    amount_money: { amount: dollarsToCents(amount), currency: "USD" },
    reason,
  });
  return { refundId: refund.refund?.id, simulated: false, amount };
}
...
export async function refundDeposit(booking) {
  ...
  // Look up the invoice's payment (via the order tenders), then refund the deposit.
  const paymentId = await getInvoicePaymentId(booking.square_invoice_id);
  if (!paymentId) {
    // Marked paid but Square has no captured payment (manual "Mark as paid",
    // cash/check, old test data). Nothing to refund through Square — don't throw.
    console.warn(
      `[square] No captured payment on invoice ${booking.square_invoice_id} for booking #${booking.id} — nothing to refund.`
    );
    return { refundId: null, simulated: false, noPayment: true };
  }
  const refund = await squareFetch("/v2/refunds", {
    idempotency_key: randomUUID(),
    payment_id: paymentId,
