...
export async function createBookingInvoice(booking) {
  ...
  // 1) Create an order with the two line items.
  ...
  // 2) Create a draft invoice against the order, then publish it.
  const invoice = await squareFetch("/v2/invoices", {
    ...
    invoice: {
      ...
      payment_requests: [{ request_type: "BALANCE", tipping_enabled: false }],
